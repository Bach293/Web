  ...
  <protected-layout>
    <div class="col p-4 bg-f9f9f9">
      ...
      >
        ...
        <div class="row p-3 align-items-center">
          <div class="col-lg-2 col-sm-4 col-12 margin_validate">
            {{ data.price_single_label }}<span class="font-color_red">(*)</span>
          </div>
          <div
        ...
        <div class="row p-3 align-items-center">
          ...
          <div class="col-lg-9 col-sm-8 col-12">
            <select class="form-select" @change="updateSelectedCity">
              <option selected disabled>{{ data.delivery_placeholder }}</option>
              <option
          ...
          <div class="row p-3 align-items-center">
            ...
            <div class="col-lg-2 col-sm-6 col-6 py-3">
              <input
                type="text"
                class="form-control"
                :placeholder="data.color_name_label"
              />
            </div>
            <div class="col-1">
              <i
        ...
        <div class="row p-3 align-items-center mt-5">
          ...
          <div class="col-lg-3 col-sm-4 col-12 py-3">
            <button
              ...
              @click="actionToPerform.value = 'draft'"
            >
          ...
          <div class="col-lg-3 col-sm-5 col-12 py-3">
            <button
              ...
              data-bs-toggle="modal"
              data-bs-target="#exampleModalToggle"
              @click="actionToPerform.value = 'publish'"
            >
...
const content = ref("<br><br><br><br><br><br><br><br><br><br>");
const actionToPerform = ref(null);
...
const fetchCategoriesAndCities = async () => {
  try {
    ...
    toast.error("Failed to load cities and categories", {
      timeout: 2000,
    });
  }
};

onMounted(fetchCategoriesAndCities);

const updateSelectedCategory = (event) => {
  if (selectedCategory.value != null) {
    selectedCategory.value = parseInt(event.target.value, 10);
  }
};
...
const onSubmit = async () => {
  const payload = {
    name: data.value.name_product,
    sku: data.value.sku,
    slug: data.value.slug,
    description: content.value,
    discount_amount: parseFloat(document.getElementById('exampleFormControlInput1').value),
    price: parseFloat(data.value.price_single),
    status: data.value.status == 'Đang bán' ? 1 : 2,
    stock_quantity: 50,
    trademark: data.value.brand,
    warranty_period: data.value.warranty,
    type: data.value.type,
    ship_from: selectedCity.value, 
  ...
  try {
    let response;
    if (actionToPerform.value == 'draft') {
      response = await productsService.createDraft(payload);
      toast.success('Draft saved successfully');
    } else if (actionToPerform.value == 'publish') {
    ...
    toast.error(`Failed to ${actionToPerform.value}: ${error.message}`);
  }
};
...
function collectVariants() {
  ...
  return Array.from(variantElements).map(element => {
    return {
      ...
      price: parseFloat(element.querySelector('.price-input').value)
    };
  });
}
...
const schema = Yup.object().shape({
  price_single: Yup.number().required("Vui lòng nhập đơn giá"),
...
function handleImageUpload(event) {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    reader.onload = (e) => {
      images.value.push({
        url: e.target.result,
        name: `${file.name.split('.')[0]}-${generateRandomString(8)}.${file.name.split('.').pop()}`
      });
    };
